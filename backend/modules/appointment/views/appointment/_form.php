...
<div class="appointment-form form-inline">
    ...
    <?= $form->field($model, 'tug')->dropDownList(ArrayHelper::map(Vessel::findAll(['status' => 1, 'vessel_type' => 2]), 'id', 'vessel_name'), ['prompt' => '-Choose a Tug-', 'disabled' => $model->isNewRecord ? FALSE : $model->status == 0 ? TRUE : $flag_tug]) ?>

    <?= $form->field($model, 'barge')->dropDownList(ArrayHelper::map(Vessel::findAll(['status' => 1, 'vessel_type' => 3]), 'id', 'vessel_name'), ['prompt' => '-Choose a Barge-', 'disabled' => $model->isNewRecord ? FALSE : $model->status == 0 ? TRUE : $flag_barge]) ?>
